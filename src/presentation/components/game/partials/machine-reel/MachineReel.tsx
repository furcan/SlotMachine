...
function MachineReel({ classNamePrefix, slotData, refReel, refsSymbols, achievedIndexes, alignment, hasEnded }: IMachineReel): JSX.Element {
  return (
    <div className={`${classNamePrefix}__reel ${classNamePrefix}__reel--${getReelsAlignmentsDescription(alignment)}`} ref={refReel}>
      {slotData?.map((symbol: ESymbols, index: number) => {
        ...
        return (
          ...
          >
            <img className={`${classNamePrefix}__reel__symbol__image`} src={getSymbolsImagesAsSrc(symbol)} width="90" height="77" alt={constants.app.name} />
          </div>
        );
      })}
    </div>
